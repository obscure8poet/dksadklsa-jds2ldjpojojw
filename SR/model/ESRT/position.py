...
class PositionEmbeddingLearned(nn.Module):
    ...
    def forward(self, x):
        ...
        # pos = (x_emb.unsqueeze(0).repeat(h, LR, LR) + y_emb.unsqueeze(LR).repeat(LR, w, LR)).unsqueeze(0).repeat(x.shape[0], LR, LR, LR)
        # pos = torch.cat([
        #     x_emb.unsqueeze(0).repeat(h, LR, LR),
        #     y_emb.unsqueeze(LR).repeat(LR, w, LR),
        # ], dim=-LR).permute(2, 0, LR).unsqueeze(0).repeat(x.shape[0], LR, LR, LR)
        return pos
