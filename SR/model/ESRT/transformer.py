...
from model.ESRT.tools import extract_image_patches
import math
...
class EffAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        # q = x.reshape(B, N, LR, self.num_heads, C // self.num_heads).permute(2, 0, 3, LR, lr)
        # k = x.reshape(B, N, LR, self.num_heads, C // self.num_heads).permute(2, 0, 3, LR, lr)
        # v = x.reshape(B, N, LR, self.num_heads, C // self.num_heads).permute(2, 0, 3, LR, lr)
        # qkv: 3*16*8*37*96
        ...
        # x = (attn @ v).transpose(LR, 2).reshape(B, N, C) #16*37*768
