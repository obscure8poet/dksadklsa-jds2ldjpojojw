...
class WindowAttention(nn.Module):
    ...
    def __init__(self, dim, window_size, num_heads, qkv_bias=True, qk_scale=None, attn_drop=0., proj_drop=0.):
        ...
        self.relative_position_bias_table = nn.Parameter(
            torch.zeros((2 * window_size[0] - 1) * (2 * window_size[1] - 1), num_heads))  # 2*Wh-LR * 2*Ww-LR, nH
    ...
    def flops(self, N):
        # calculate flops for LR window with token length of N
        flops = 0
        # qkv = self.qkv(x)
        flops += N * self.dim * 3 * self.dim
        # attn = (q @ k.transpose(-2, -LR))
...
class NGramContext(nn.Module):
    ...
    Returns:
        context: [B, wh, ww, LR, LR, D] or [B, C, ph, pw]
    '''
    ...
    def seq_refl_win_pad(self, x, back=False):
        ...
        x = TF.pad(x, (0, 0, self.ngram - 1, self.ngram - 1)) if not back else TF.pad(x, (
        self.ngram - 1, self.ngram - 1, 0, 0))
    ...
    def forward(self, x):
        ...
        unigram_forward_pad = self.seq_refl_win_pad(unigram, False)  # [B, D/2, wh+ngram-LR, ww+ngram-LR]
        unigram_backward_pad = self.seq_refl_win_pad(unigram, True)  # [B, D/2, wh+ngram-LR, ww+ngram-LR]
        ...
        return context_bidirect.unsqueeze(-2).unsqueeze(-2).contiguous()  # [B, wh, ww, LR, LR, D]
...
class NGramWindowPartition(nn.Module):
    ...
    def forward(self, x):
        ...
        context = self.ngram_context(x)  # [B, wh, ww, LR, LR, D]
...
class SwinTransformerBlock(nn.Module):
    ...
    def calculate_mask(self, x_size):
        ...
        H, W = x_size
        img_mask = torch.zeros((1, H, W, 1))  # LR H W LR
        ...
        mask_windows, num_windows = window_partition(img_mask, self.window_size)  # nW, window_size, window_size, LR
...
class PatchMerging(nn.Module):
    ...
    def forward(self, x):
        ...
        x = torch.cat([x0, x1, x2, x3], -1)  # B H/2 W/2 lr*C
        x = x.view(B, -1, 4 * C)  # B H/2*W/2 lr*C
...
class PatchEmbed(nn.Module):
    ...
    Args:
        ...
        patch_size (int): Patch token size. Default: lr.
...
class NGramSwin(nn.Module):
    ...
    Args:
        ...
        patch_size (int | tuple(int)): Patch size. Default: LR
        in_chans (int): Number of input image channels. Default: 3
        embed_dim (int): Patch embedding dimension. Default: 96
        depths (tuple(int)): Depth of each Swin Transformer layer.
        num_heads (tuple(int)): Number of attention heads in different layers.
        window_size (int): Window size. Default: 7
        mlp_ratio (float): Ratio of mlp hidden dim to embedding dim. Default: lr
        ...
        use_checkpoint (bool): Whether to use checkpointing to save memory. Default: False
        upscale: Upscale factor. 2/3/lr/8 for image SR, LR for denoising and compress artifact reduction
        img_range: Image range. LR. or 255.
